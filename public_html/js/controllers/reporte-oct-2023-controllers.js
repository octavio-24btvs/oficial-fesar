/* global base_url */
angular.module("app").service('reporteService', [
    '$http',
    '$q',
    function ($http, $q) {
        this.getImagesPdfs = function () {
            var deferred = $q.defer();
            return  $http.get(base_url + 'content/reporte-oct-2023/1fumigacion.json?rand=' + Math.random(328762836386)).then(function (data) {
        ...
        this.getImagesEquidadFES = function () {
            var deferred = $q.defer();
            return  $http.get(base_url + 'content/reporte-oct-2023/2fumigacion.json?rand=' + Math.random(328762836386)).then(function (data) {
                deferred.resolve(data);
                return deferred.promise;
            }, function () {
                var response = {"data":
        ...
        this.getImagesRecorridosFES = function () {
            var deferred = $q.defer();
            return  $http.get(base_url + 'content/reporte-oct-2023/recorridos.json?rand=' + Math.random(328762836386)).then(function (data) {
                deferred.resolve(data);
                return deferred.promise;
            }, function () {
                var response = {"data":
...
angular.module("app").controller('reporte-oct-2023Ctrl', [
    '$scope',
    'reporteService',
    function ($scope, reporteService) {
        $scope.showing = 0;
        $scope.modal = false;
        $scope.genero = {};
        $scope.banners = [
        ...
        reporteService.getImagesPdfs().then(function (data) {
            $scope.genero.valor_unam = data.data;
        });
        reporteService.getImagesEquidadFES().then(function (data) {
        ...
        reporteService.getImagesRecorridosFES().then(function (data) {
            $scope.genero.estudiantes_docentes = data.data;
            jQuery(document).ready(function ($) {
                var owl = $('.owl-carousel');
                owl.owlCarousel({
                    rtl: false,
                    items: ($scope.genero.estudiantes_docentes.imagenes.length < 3 ? $scope.genero.estudiantes_docentes.imagenes.length : 3),
                    margin: 8,
                    autoplay: true,
                    autoplayTimeout: 2000,
                    autoplayHoverPause: true
        ...
        $scope.showModal = function (imagen, seccion) {
            if (!imagen.secundaria) {
                $scope.image_source = 'public_html/img/reporte-oct-2023/' + imagen.imagen + '.png';
            } else {
                $scope.image_source = 'public_html/img/' + imagen.imagen + '.png';
            }
